...
export async function deployToCFPages(
  ...
  // 4. Ask CF which hashes are missing — only upload those. If
  //    check-missing 5xxs (CF-side worker bug we've seen), we treat the
  //    entire manifest as missing and let the upload step decide.
